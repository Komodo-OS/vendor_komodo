...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/komodo/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Komodo model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch komodo_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/komodo-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.komodo.device | grep -q "$KOMODO_BUILD"); then
...
function githubremote()
{
    ...
    git remote add github https://github.com/KomodoOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.komodo.device | grep -q "$KOMODO_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.komodo.device | grep -q "$KOMODO_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $KOMODO_FIXUP_COMMON_OUT ]; then
